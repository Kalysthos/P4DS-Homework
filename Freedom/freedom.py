...
freedom = pd.DataFrame(pd.read_excel('Freedom/Freedom/'+table_xls[0])[table_xls[0][5:9]+" Score"].append(pd.Series([0]), ignore_index=True).loc[:185])
freedom.columns = [2013]
for k,i in enumerate(table_xls[1:]):
    ind = pd.DataFrame(pd.read_excel('Freedom/Freedom/'+i)[i[5:9]+" Score"].append(pd.Series([0]), ignore_index=True).loc[:185])
    ind.columns=[2014+k]
